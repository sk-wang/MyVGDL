...
class SpawnPoint(SpriteProducer):
    ...
    def update(self, game):
        ...
        if self.total and self.counter >= self.total:
            killSprite(self, None, game)
class CounterClockwiseSprite(VGDLSprite):
    speed = 0.5
    direction = 0
    def update(self, game):
        #print self.is_stepBack
        if(self.is_stepBack == True):
            self.direction+=1
            if(self.direction >= 4):
                self.direction = 0
        VGDLSprite.update(self, game)
        self.physics.activeMovement(self, BASEDIRS[self.direction])
        self.is_stepBack = False
class ClockwiseSprite(VGDLSprite):
    speed = 0.5
    direction = 0
    def update(self, game):
        #print self.is_stepBack
...
#@author:Hao Wang
class LinkDead(Termination):
    """ Game ends when the sum of all sprites of types 'stypes' hits 'limit'. """
    def __init__(self, win=False, score=-1):
        self.win = win
        self.score = score

    def isDone(self, game):
        if game.sprite_groups['link'] == []:
